<form wire:submit.prevent="submit" method="POST">
    <div class="bg-white  p-10 m-10 rounded">
        <div class="flex space-x-3 justify-center items-center mb-4">
            ...
            <h2 class="text-center text-2xl font-bold text-gray-800">Crear Doctor</h2>
        </div>
        <div class="grid grid-cols-2 gap-4">

            <x-input-text wire:model="data.nombre_doctor" type="text" name="nombres" required :label="'Nombres *'"
                value="{{ old('nombre_doctor') ? old('nombre_doctor') : '' }}">
            ...
            <x-input-text wire:model="data.apellido_doctor" type="text" name="apellidos" required :label="'Apellidos *'"
                value="{{ old('apellido_doctor') ? old('apellido_doctor') : '' }}">
            ...
            <x-input-text wire:model="data.identidad" :label="'N° Identidad *'" type="text" id="identidad"
                name="identidad" required value="{{ old('identidad') ? old('identidad') : '' }}">
            ...
            <x-input-text wire:model="data.email" :label="'Correo electronico *'" type="text" id="correo" name="correo"
            ...
            <x-input-text wire:model="data.direccion" :label="'Dirección *'" type="text" id="direccion" name="direccion"
                required value="{{ old('direccion') ? old('direccion') : '' }}">
            ...
            <x-input-text wire:model="data.telefono" :label="'Telefono/Celular *'" type="text" id="telefono"
                name="telefono" required value="{{ old('telefono') ? old('telefono') : '' }}">
                @error('data.telefono')
                    <span class="text-red-500" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="flex flex-col">
                ...
                <select wire:model="data.especialidad" class="rounded-xl border-2 p-2 form-control focus:outline-none"
                    required>
                    <option wire:key="Medico General" value="">Seleccione una especialidad</option>
        ...
        <div class="flex justify-center gap-2">
            ...
            <x-button-submit type="reset">
                Borra todo
            </x-button-submit>
            <x-button-submit type="submit">
                <svg wire:loading.delay class="animate-spin w-6 h-6 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
